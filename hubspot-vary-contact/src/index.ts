...
const VARY_CONTACT_URL = process.env.VARY_CONTACT_URL || process.env.VARY_TASK_URL || "https://varyws05.enygea.com/test/1/contact";
const VARY_CUSTOMER_URL = process.env.VARY_CUSTOMER_URL || "https://varyws05.enygea.com/test/1/customer"; // for lookup by SIRET
...
// =============================
functions.http("hubspotVaryContactUpsert", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const overrideRaw = (payload.is_blocked ?? req.query?.is_blocked);
  ...
  const hsConsentOptIn =
    ...
    if (!lastName && !email) {
      return res.status(400).json({ message: "Missing required identity: provide at least lastname or email" });
    }
    ...
    return res.status(500).json({ message: "Erreur lors du traitement du contact", details: safeError(error) });
  }
});
